...
export function formatDurationType(durationToFormat) {
	if (durationToFormat) {
		let valueInSeconds = durationToFormat.substring(durationToFormat.indexOf('.') + 1, durationToFormat.length - 1);
		return valueInSeconds ? formatDuration(intervalToDuration({ start: 0, end: valueInSeconds * 1000 })) : '-';
	} else {
		return '-';
	}
}

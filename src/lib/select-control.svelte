<script>
	...
	function selectOption(event) {
		...
		dirty = true;
		const eventValue =
			typeof options.data[0]['value'] === 'object' ? JSON.parse(event.target.value) : event.target.value;
		if (event.target.checked) {
			...
			if (multiple) {
				valueToDispatch = value.filter(val =>
					typeof val === 'object' ? val[options.idField] !== eventValue[options.idField] : val !== eventValue
				);
			} else {
				valueToDispatch = [];
			}
		...
		validateRequired();
		dispatch('select', {
			value: multiple ? valueToDispatch : valueToDispatch.length > 0 ? valueToDispatch[0] : null,
		});
	}
				...
				<CheckboxControl
					...
					checked={value &&
						(typeof option.value === 'object'
							? value.find(val => val[options.idField] === option.value[options.idField]) !== undefined
							: value.includes(option.value))}
					value={typeof option.value === 'object' ? JSON.stringify(option.value) : option.value}
					on:change={event => selectOption(event)}
					>{typeof option.value === 'object' ? option.value[options.nameField] : option.name}</CheckboxControl
				>
			</div>
		{/each}
	</div>

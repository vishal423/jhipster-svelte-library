<script>
	...
	onMount(() => {
		const ref = setTimeout(() => dispatch('timeout', { message, index }), timeout)
...
<div class="absolute w-1/2 z-20">
	...
	>
		...
		>
			<Icon icon={contextualColor === 'success' ? faCheck : faExclamationTriangle} />
		</div>
		<div class="px-5 py-3" data-testid="{testId}-{contextualColor}" {...$$restProps}>
			{message}
		</div>
		{#if closeable}
			<button
				...
				on:click|preventDefault={closeAlert}><span aria-hidden="true">&times</span></button
			>
		{/if}
	</div>
</div>

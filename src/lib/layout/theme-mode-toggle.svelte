<script>
	...
	function loadThemePreference() {
		if (browser) {
			...
			darkMode = (storage && storage.getItem('svl-theme-mode') === 'dark') || false;
		}
...
{#await loadThemePreference() then response}
	<div class="align-middle">
		<label class="relative inline-block w-12 h-6 border-0 rounded-3xl cursor-pointer bg-black">
			<input
				class="opacity-0"
				type="checkbox"
			...
			/>
			<span class="w-6 h-6">
				<Icon classes="absolute top-1/2 transform -translate-x-2 -translate-y-1/2 text-white" icon={faMoon} />
				<Icon
